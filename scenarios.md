...
Bu tezin temel hedefi, kaba kuvvet gürültüsüyle kalabalık bir sürüyü istatistiksel yoğunluktan faydalanarak olasılıksal olarak vurmak değil; kurbanların reaktif frekans atlama (Markovian hopping) örüntüsünü ve fiziksel hareketlerini çözebilen gerçek bir **saldırı zekası (state-aware tracking)** oluşturmaktır. Önerilen senaryoda ise $C=8$ kullanılabilir frekans kanalı (LoRaWAN EU868), $J=1$ jammer ve $M=3$ İHA yer almaktadır. Bu yeni kurgu, hem jammer spektral kaplama oranını baseline çalışmaya paralel olarak **%12.5'e ($1/8$)** düşürmekte hem de İHA'ların zeki takibini en üst düzeyde test etmektedir. Bu doğrultuda asıl belirleyici olan **rastgele hedef vurma olasılığı** ve **kurbanın kaçış yedekliliğidir (escape redundancy)**:
1. **Rastgele Hedef Vurma Olasılığının Düşürülmesi:** Baseline kurguda jammer'ın rastgele bir atışla en az bir İHA'yı vurma olasılığı **%84.6** iken, önerilen 8 kanallı senaryoda bu oran **%37.5'e ($3/8$)** indirilmiştir. Yani rastgele bir jammer'ın hedefleri ıskalama olasılığı %15.4'ten **%62.5**'e çıkarılmıştır.
2. **UAV Kaçış Yedekliliği (Escape Redundancy) ve Çözüm Kümesi:** $C=8$, $M=3$ ve $J=1$ kurgusunda 3 İHA kanalları temiz olarak paylaştığında geriye 5 boş yedek kanal kalır. Bu da İHA'lar için kaçış yedekliliği sağlar.

Bu izole kurguda İHA'lar reaktif olarak (sadece karıştırıldıklarında) kaçtıkları için, jammer'ın kurbanları sürekli bloke edebilmesi için **İHA'ların 8 kanallı Markov geçiş örüntülerini ve dinamik seyrüsefer yollarını aktif olarak tahmin etmesi** zorunludur. Aksi takdirde, durumsuz QJC modelinde olduğu gibi jammer statik bir kanala odaklandığında İHA'yı en fazla 1 kez karıştırır; İHA hemen boş yedek kanallardan birine kaçarak orada sabit kalır ve jammer eski kanalda boş yayın yapmaya devam eder. Nitekim, durumsuz QJC modelinin bu çok kanallı ve çok İHA'lı kurguda takip başarısının tamamen çökmesi, önerilen durum-farkında PPO modelimizin başarısının tamamen "tahmin zekasına" dayandığını bilimsel olarak kanıtlamaktadır.
...
- **Alan ve Varlıklar:** 1000 _m ×_ 1000 _m_ engelsiz (obstacle-free) alan; 30 sabit IoT düğümü, 3 İHA ($M=3$) ve 1 Zeki Jammer. 

- **Seyrüsefer ve Rota Planlama:** Seyahat sürelerini ve işlem gecikmelerini azaltmak amacıyla APF tabanlı çarpışma önleme itici kuvvetleri devre dışı bırakılmış, doğrudan doğrusal rotalar izlenmiştir. Düğümler arası rota atamasında ise statik coğrafi kümeleme (clustering) yerine dinamik ve işbirlikli bir **Ortak Ziyaret Edilmeyen Düğümler Havuzu (Shared Unvisited Pool)** kurgulanmıştır. Her İHA, o anda diğer İHA'lar tarafından hedeflenmemiş en yakın düğüme yönelir. Bu dinamik paylaşım, jammerın bulunduğu bölgeye sürekli aynı İHA'nın giderek yıpranmasını önler ve jammerın tek bir kurbana kilitlenmesini engeller. İHA'lar riskli bölgedeki düğümleri sırayla ve dönüşümlü olarak ziyaret eder.

- **İletişim Dinamikleri:** İHA, SINR seviyesi eşiğin ($< 0$ dB) altına düştüğünde statik bir kurban olarak kalmamakta, Markov geçiş matrisine bağlı istatistiksel bir örüntü ile reaktif olarak kanal değiştirmektedir (8 kanallı EU868 LoRaWAN bandı). 

- **Fiziksel Kısıtlar ve Enerji:** Hem yer düğümlerinin hem de İHA vericilerinin ortak bir LoRa altyapısı kullandığı varsayılarak, iletim gücü ($P_{tx} = 14$ dBm / $0.025$ W) ve uniform Güç Amplifikatörü (PA) verimliliği ($\eta_{PA} = 0.298$), **Semtech SX1261** veri sayfasına (datasheet) uygun şekilde belirlenmiştir. Saldırganın (jammer) güç tüketim maliyeti ise doğrudan anlık RF çıkış gücü ($0.1\text{ W} - 1.0\text{ W}$) olarak modellenmiştir. Saldırgan; normalize mesafe (RSS proxy), spektrum doluluğu ve bu güç kısıtlarını dikkate almak zorundadır. 

Bu sayede jammer’ın yalnızca sınırsız güçle yayın yapması engellenmiş, "doğru anlarda derin SINR düşüşleri ($SINR \ll 0$)" yaratarak PPO-LSTM modeliyle etkili ve enerji-verimli saldırılar öğrenmesi garanti altına alınmıştır. 
...
|**Bu Çalışma (Önerilen Se-**<br>**naryo)**|**1000m x 1000m**,<br>Engelsiz (Obstacle-<br>Free), Sabit İrtifa:<br>100m|**3 İHA ($M=3$),**<br>**30 IoT**<br>**Cihazı,**<br>**1 Durum-**<br>**Farkında**<br>**(State-**<br>**Aware) Zeki Jam-**<br>**mer**|**PPO,**<br>**DQN,**<br>**PPO-**<br>**LSTM vs. QJC (Base-**<br>**line).** Eş-kanal girişimi ve paylaşılan kanallarda zeki tehdidin Markov örüntüsü takibi ve APF devre dışı bırakılmış işbirlikli seyrüsefer. |
