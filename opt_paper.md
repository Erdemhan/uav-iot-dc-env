...
Dağıtık pekiştirmeli öğrenme eğitimleri, $1$ adet Head Node (koordinatör) ve $12$ adet fiziksel Worker Node'dan oluşan yerel bir Ray kümesinde (Ray Cluster) koşturulmuştur.

### 4.1. STRICT_PACK Kaynak İzolasyonu
İşçi makinelerin her biri $22$ CPU thread'i ve $1$ adet NVIDIA RTX GPU barındırmaktadır. Denemelerin birbirlerinin kaynaklarını gasp ederek (CPU/VRAM çekişmesi) eğitim sürelerini yapay olarak uzatmasını ve ASHA budayıcısını yanıltmasını önlemek için şu strateji izlenmiştir:
* Her deneme için `num_workers = 10` (10 adet CPU üzerinde çalışan env runner) ve `num_gpus = 1.0` (öğrenme için ekran kartının tamamı) tahsis edilmiştir. 
