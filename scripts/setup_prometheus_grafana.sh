...
# 5.5 Configure allow_embedding and anonymous auth via environment variables in /etc/default/grafana-server
echo "Configuring security and anonymous auth overrides in /etc/default/grafana-server..."
sudo tee -a /etc/default/grafana-server > /dev/null << 'EOF'

# Ray Dashboard Embedding Overrides
GF_SECURITY_ALLOW_EMBEDDING=true
GF_AUTH_ANONYMOUS_ENABLED=true
GF_AUTH_ANONYMOUS_ORG_ROLE=Viewer
EOF
echo "[OK] Grafana security and anonymous auth configured successfully via environment overrides."
