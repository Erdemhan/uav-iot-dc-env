...
try:
    ...
    # We target PPO-LSTM jobs, and specifically NOT DQN (Job 09000000 or any entrypoint with DQN)
    target_jobs = []
    for j in active_jobs:
        if "DQN" in j.entrypoint:
            print(f"[KEEPING] Keeping active DQN Job ID: {j.job_id} ({j.entrypoint}) running.")
        elif "PPO-LSTM" in j.entrypoint:
            target_jobs.append(j)
            
    if not target_jobs:
        print("No duplicate/active PPO-LSTM jobs found to stop.")
        sys.exit(0)
        
    print(f"\nFound {len(target_jobs)} active PPO-LSTM jobs. Stopping them now...\n")
    ...
    print("\n[COMPLETE] Cleaned up duplicate PPO-LSTM jobs.")
