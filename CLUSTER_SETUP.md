...
ray start --head --port=6379 --dashboard-host=0.0.0.0 --num-cpus=0 --num-gpus=0 --node-manager-port=10004 --object-manager-port=10002 --dashboard-agent-listen-port=10003 --min-worker-port=10010 --max-worker-port=10099 --system-config='{"num_heartbeats_timeout":600}'
```
Başarıyla çalıştığında konsolda şu çıktıyı göreceksiniz:
```text
Ray runtime started.
To add another node to this Ray cluster, run:
  ray start --address='192.168.1.50:6379' --node-manager-port=10004 --object-manager-port=10002 --dashboard-agent-listen-port=10003 --min-worker-port=10010 --max-worker-port=10099 --system-config='{"num_heartbeats_timeout":600}'
```
